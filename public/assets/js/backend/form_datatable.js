...
const datatableSetup = {
    ...
    setup() {
        ...
        onMounted(async () => {
            ...
            fetchData(`/demoadmin/api/form-datatable/${formType}`, queryParams, callback);




            // try {
            //     const response = await axios.get(`/demoadmin/api/form-datatable/${formType}`, {
            //         params: queryParams
            //     });
        ...
        const openDetail = async (id) => {
            try {
                const api_response = await axios.post(baseUrl + 'demoadmin/getFormData', {
                    id: id
                }, {
                    headers: {
        ...
        const updateStatus = async (newValue, item) => {
            ...
            try {
                const response = await axios.post(baseUrl + 'demoadmin/updateStatus', {
                    id: item.id,
                    status: newValue
                });
                yuanUI.log("response:", response);
        ...
        const sendFilter = async () => {
            ...
            await fetchData(`/demoadmin/api/form-datatable/${formType}`, queryParams, (response) => {
                // 你可以在这里处理响应数据或执行回调
        ...
        const clearFilters = async () => {
            ...
            await fetchData(`/demoadmin/api/form-datatable/${formType}`, null, (response) => {
                // 你可以在这里处理响应数据或执行回调
                console.log('Callback data:', response);
                const paper = response.paper;
                totalPages.value = paper.pageCount;
        ...
        const exportCsv = () => {
            window.location.href = `${baseUrl}demoadmin/export-csv/${formType}`;
        }
